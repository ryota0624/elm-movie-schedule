...
type alias Movies = List Movie
type Msg = Nil | Store Movie
...
subscriptions state = Sub.batch [ storeFromPort Store ]
...
update msg state =
    ...
    Store movie ->
      let 
        nextState = { state | movies = Maybe.map(\movies -> movie::movies)(state.movies) }
      in 
