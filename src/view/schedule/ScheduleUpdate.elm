...
update msg state =
  case msg of
    NoMsg ->
      ( state, Cmd.none )
    StoreSchedule schedule ->
      ( { state | schedule = schedule }, Cmd.none )
    ClickMovie movie ->
    ...
    ClickPrevDate date ->
      let 
        (movedDate, movedTime) = timeJumpDay date -1
      in ( { state | currentDate = Maybe.Just(movedDate) }, onChangePreviewTime movedTime )
    ClickNextDate date ->
      let 
        (movedDate, movedTime) = timeJumpDay date 1
      in ( { state | currentDate = Maybe.Just(movedDate) }, onChangePreviewTime movedTime )
...
timeJumpDay date num =
  let 
    time = Date.toTime date |> Time.inMilliseconds
    movedTime = time + (num * 60 * 60 * 24 * 1000)
  in
    (Date.fromTime <| movedTime, movedTime)

now: Cmd Msg
now = Task.perform SetTime Time.now